...
def remove_word_strain(strain):
    if not pd.isnull(strain) and (strain != "na"):
...
class AddStrainPlugin:
    def input(self, infile):
        self.parameters = PyIO.readParameters(infile)
        self.lineage_file = PyPluMA.prefix()+"/"+self.parameters["lineagefile"]
        self.assembly_summary = PyPluMA.prefix()+"/"+self.parameters["assemblysummary"]
    ...
    def output(self, outfile):
        ...
        lineage_df = pd.read_table(self.lineage_file, sep="\t", dtype=str)
        lineage_df = lineage_df[['tax_id', 'strain', 'assembly', 'assembly_accession', 'phylum', 'class', 'order', 'family', 'genus', 'species']]
        lineage_df = lineage_df.rename(index=str, columns={"strain":"scientific_name"})
        assembly_summary_df=pd.read_table(self.assembly_summary, sep="\t", dtype=str, skiprows=1)
        ...
        # ---------------------------------------------------------------------------------------------------------------------
        merged_df.to_csv(self.outputfile, index=False, sep="\t")
